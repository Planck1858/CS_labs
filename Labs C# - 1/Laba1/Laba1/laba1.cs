...
namespace Laba
{
    class Laba1
    {
        ...
        public void Parse(string filePath)
        {
            ...
            var textChapter = System.IO.File.ReadAllText(FilePathTextChapter);
            textChapter = textChapter.ToLower();

            charTextSorted = CharFreqSorter(inputText);
            var chapterEncr = Encrypter(textChapter, 3); //шифрование главы
            charEncryptedSorted = CharFreqSorter(chapterEncr);
            decryptedStringCaesar = Decrypter(chapterEncr, charTextSorted, charEncryptedSorted);

            bigramFreqSortedText = BigramFreqSorter(inputText);
            bigramFreqSortedEncrypted = BigramFreqSorter(chapterEncr);
            decryptedStringBigram = DecrypterBigram(chapterEncr, bigramFreqSortedText, bigramFreqSortedEncrypted);
